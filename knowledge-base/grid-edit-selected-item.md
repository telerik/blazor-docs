...
1. Use one-way data binding for the [SelectedItems]({%slug components/grid/selection/rows%}#selected-rows) parameter and in the handler for the [SelectedItemsChanged]({%slug components/grid/selection/rows%}#selecteditemschanged) populate the instance of the class created in the previous point.
