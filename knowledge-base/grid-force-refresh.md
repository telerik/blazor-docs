...
When using manual operations through the [OnRead event](/blazor-ui/components/grid/manual-operations), call the component's [`Rebind()` method]({%slug grid-refresh-data%}#rebind-method). This will force the component to fire its `OnRead` event.
