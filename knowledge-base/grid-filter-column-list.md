...
1. Define a [`FilterCellTemplate` or a `FilterRowTemplate`]({%slug grid-templates-filter%}), depending on the Grid `FilterMode`. Add a [MultiSelect]({%slug multiselect-overview%}) or another dropdown component in the template, according to your preferences. It's also possible to use [CheckBoxes]({%slug checkbox-overview%}).
