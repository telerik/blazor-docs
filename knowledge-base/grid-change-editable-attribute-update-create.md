...
* Use an [editor template](/blazor-ui/components/grid/templates#edit-template) and add logic to it that renders an actual editor only when needed (for example, the edited item has no ID, so it is a Create operation for a new item).
