...
* The PDF document container (`div.k-canvas`) renders a `k-enable-text-select` CSS class.
* The **Zoom** toolbar button group (`div.k-button-group`) render `k-toolbar-button-group` and `k-button-group-solid` CSS classes.
