...
- TileLayout - introduced identifier of the TileLayout item. The `OnResize` event will receive argument of type `TileLayoutResizeEventArgs`. The `OnReorder` event will receive argument of type `TileLayoutReorderEventArgs`. Both event arguments will point to the tile item (`args.Id`) and define if the component should re-render after the event (`args.ShouldRender`). This feature comes from a [public feature request](https://feedback.telerik.com/blazor/1489011).
