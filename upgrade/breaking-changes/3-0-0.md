...
- Button - `Primary` parameter is removed in favor of [`ThemeColor`]({%slug components/button/overview%}) of type `string`. There is a new static class `Telerik.Blazor.ThemeConstants.Button.ThemeColor` with a predefined set of theme colors. To get the old primary styling, set `ThemeColor="@(ThemeConstants.Button.ThemeColor.Primary)"` for the Button.
