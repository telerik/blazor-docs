...
>caption Setting Map URL template up to version 13.3.0 and after version 14.0.0
...
The removed `Dark` and `Light` values have no one-to-one replacements. If the button only needs a supported semantic theme color, choose a value from the current `ThemeColor` options based on the intended action. If the application must preserve the previous visual appearance, use the Button `Class` parameter with custom CSS instead.
