...
* Components that already set their apperance parameters (for example, `Primary` `ThemeColor` or `Large` `Size`) maintain their styling.
...
The above information and code snippets use the Button component as an example, but the same idea applies to all components, which use some or all of the [appearance parameters](#default-apperance-settings):
