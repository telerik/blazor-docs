...
* `ToolBarSeparator` - to separate two items with a solid line, add the `<ToolBarSeparator>` tag between them. The separator exposes `Overflow` (`ToolBarItemOverflow` enum, default`Auto`) parameter to specify whether the separator will be hidden when the adaptive ToolBar is resized.
