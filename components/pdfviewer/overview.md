...
| `Zoom` | `object` <br /> (`1.25m`) | The current zoom level. Accepts a `decimal` ratio (e.g., `1.0m` for 100%, `2.0m` for 200%) or a `PdfViewerZoomLevelType` enum value (`ActualWidth`, `FitToWidth`, or `FitToPage`) for semantic zoom modes. Numeric values are clamped to [`MinZoom`, `MaxZoom`]. Use the parameter with two-way binding or with a [`ZoomChanged` event handler](slug:pdfviewer-events#zoomchanged). |
