...
| `AllowCustom` | `bool` | Defines if the user can enter a custom value that is among the dropdown items. The custom value becomes the component `Value`. If enabled, the `Value` must be of type `string`. See [ComboBox Custom Values]({%slug components/combobox/custom-value%}) for details and examples. |
