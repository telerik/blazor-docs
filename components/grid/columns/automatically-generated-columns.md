...
The Grid allows you to automatically generate a column for each public property of a given model.
...
If the Grid has no explicitly defined columns the `GridColumns` tag is not required.
...
By default the Automatically Generated Columns are rendered after the manually generated ones.
 ...
 * The properties that do not have a `setter` will not be editable.
...
>tip To enable horizontal scrolling the `ColumnWidth` parameter has to be set so that the sum of all columns is greater than the Grid width. More information can be found in the [Column Width]({%slug grid-columns-width%}) article.

>tip To use the custom attributes the `System.ComponentModel.DataAnnotations` using statement has to be present in your razor page. You can use all validation attributes in conjunction with the custom attributes listed above.
