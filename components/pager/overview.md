...
| ----------- | ----------- | ----------- |
|`Adaptive` | `bool` | Defines whether pager elements should be changed based on the screen size. If enabled, the Pager will hide its `Pager Info` and `PageSize Dropdownlist` elements when its width is not enough to render them. In the smallest resolution, the page buttons will be rendered as a select element.
