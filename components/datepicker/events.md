...
@code {
    ...
    {
        result = string.Format("BLUR fired, last value was: {0:dd/MMM/yyyy}", ( (DateTime?)theDate).Value);
    }
}
````
...
The `OnChange` event represents a user action - confirmation of the current value. It fires when:
