...
* `MinResizableWidth` - (defaults to `0`) - the minimum allowed column width during [user resizing]({%slug treelist-columns-resize%}). Unlike the string `Width` property, this one is decimal and expects pixel values.
