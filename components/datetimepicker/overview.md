...
| `Format` | `string` <br /> (`FullDateTimePattern`) | The format of the DateTimePicker's DateInput. The default value depends on `CultureInfo.CurrentCulture`. Read more at [Supported data formats by the DateInput]({%slug components/dateinput/supported-formats%}). |
