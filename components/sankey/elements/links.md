...
| --------- | ---- | ----------- |
| `ColorType` | [`SankeyLinksColorType` enum](/blazor-ui/api/telerik.blazor.sankeylinkscolortype) <br /> (`SankeyLinksColorType.Static`) | The origin of the link color: could be based on the `Color` property, on the source node color or on the target node color. |
