...
description: Overview of the DropDownButton for Blazor. Description of the component behavior, features and benefits.
...
| `Class` | `string` | Renders a custom CSS class to the main component element `<div class="k-dropdown-button">`. Use it to [override the theme styles]({%slug themes-override%}) to obtain a specific appearance, if none of the [DropDownButton appearance settings]({%slug dropdownbutton-appearance%}) can achieve this. |
...
The DropDownButton exposes a `FocusAsync` method to focus it programmatically. To use it, define a reference to the component instance with the `@ref` attribute. Be aware of the Blazor life cycle if you want to [focus the component on page load]({%slug inputs-kb-focus%}#notes).
